...
PIPELINE_MISMATCH_DIAGNOSES = {
    "pipeline_extract_coordinate_mismatch",
    "processed_coordinate_mismatch",
}
...
def load_optional_decisions(path: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    if not path.exists():
        return [], {}
...
def readiness_verdict(
    findings: list[dict[str, str]],
    accepted_worksheet_count: int,
    decision_rows_count: int,
    recovery_accepted_count: int,
) -> str:
    severity_counts = Counter(row["severity"] for row in findings)
    if severity_counts.get("P0") or severity_counts.get("P1"):
        return "not_ready"
    if recovery_accepted_count:
        return "recovery_layer_available"
    if decision_rows_count:
        return "ready_for_recovery_build"
    if accepted_worksheet_count:
        return "ready_for_decision_export"
...
def write_report(
    *,
    ...
    accepted_worksheet = [
        ...
        if as_text(row.get("reviewer_action")) in ACCEPT_ACTIONS
    ]
    recovery_accepted = [
        row
        for row in recovery_rows
        if as_text(row.get("coordinate_recovery_status"))
    ...
    verdict = readiness_verdict(
        findings,
        len(accepted_worksheet),
        len(decision_rows),
        len(recovery_accepted),
    )
    ...
    lines = [
        ...
        f"- `{verdict}`",
        "",
        "This audit is diagnostic only. It never applies coordinate overrides and never mutates raw ANNCSU coordinates.",
        "",
        ...
        "## Stage Availability",
        "",
        f"- External geocoder candidates file: {'present' if GEOCODE_CANDIDATES_CSV.exists() else 'not yet present'}",
        f"- Local-anchor candidates file: {'present' if LOCAL_ANCHOR_CANDIDATES_CSV.exists() else 'not yet present'}",
        f"- Reviewed decisions export: {'present' if REVIEWED_DECISIONS_JSON.exists() else 'not yet present'}",
        f"- Recovery layer: {'present' if RECOVERY_LAYER_CSV.exists() else 'not yet present'}",
        f"- Training set: {'present' if TRAINING_SET_CSV.exists() else 'not yet present'}",
        "",
        "## Diagnosis Counts",
        "",
    ]
    ...
    if decision_summary:
        ...
        lines.append(
            f"- decision audit findings P0/P1/P2: {dict(sorted(decision_severity_counts.items()))}"
        )
    else:
    ...
    if findings:
        ...
        for row in findings:
            ...
            lines.append(
                f"| {row['severity']} | `{row['code']}` | {detail} | {next_action} |"
            )
    else:
    ...
    lines.extend(
        [
            ...
            "",
            f"Readiness findings summary: P0={severity_counts.get('P0', 0)}, P1={severity_counts.get('P1', 0)}, P2={severity_counts.get('P2', 0)}.",
        ]
    )
...
def main() -> int:
    ...
    geocode_rows = read_optional_csv(GEOCODE_CANDIDATES_CSV)
    local_anchor_rows = read_optional_csv(LOCAL_ANCHOR_CANDIDATES_CSV)
    review_pack_rows = read_csv_rows(REVIEW_PACK_CSV)
    worksheet_rows = read_csv_rows(WORKSHEET_CSV)
    recovery_rows = read_optional_csv(RECOVERY_LAYER_CSV)
    training_rows = read_optional_csv(TRAINING_SET_CSV)
    decision_rows, _decision_meta = load_optional_decisions(REVIEWED_DECISIONS_JSON)
    decision_findings, decision_summary = (
        audit_decisions(decision_rows) if decision_rows else ([], {})
    )

    findings: list[dict[str, str]] = []
    diagnosis_counts = count_by(diagnostic_rows, "source_diagnosis")
    pipeline_mismatches = sum(
        diagnosis_counts.get(key, 0) for key in PIPELINE_MISMATCH_DIAGNOSES
    )
    ...
    if suspect_rows and diagnosis_counts.get(
        "suspect_coordinate_present_in_original_anncsu_raw", 0
    ) == len(suspect_rows):
        add_finding(
            findings,
            "P2",
    ...
    blocking_decision_findings = [
        row for row in decision_findings if row["severity"] in {"P0", "P1"}
    ]
    ...
    training_ready_count = int(
        decision_summary.get("training_ready_manual_overrides", 0) or 0
    )
    ...
    if suspect_access - manual_ready_access:
        add_finding(
            findings,
            "P2",
