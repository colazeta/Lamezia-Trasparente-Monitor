...
const uxCopyGatePath = path.join(root, 'docs/launch-v0/ux-copy-readiness-gate.md');
const sectionTaxonomyPath = path.join(root, 'docs/launch-v0/section-taxonomy-v0.md');
const civicCopyGuidePath = path.join(root, 'docs/launch-v0/civic-copy-style-guide.md');
const routeContractDocPath = path.join(root, 'docs/launch-v0/route-contract.md');
...
const uxGateDocs = [
  [uxCopyGatePath, 'checklist UX/copy readiness', 'Creare o ripristinare docs/launch-v0/ux-copy-readiness-gate.md.'],
  [sectionTaxonomyPath, 'tassonomia sezioni v0', 'Ripristinare docs/launch-v0/section-taxonomy-v0.md.'],
  [civicCopyGuidePath, 'guida copy civico prudente', 'Ripristinare docs/launch-v0/civic-copy-style-guide.md.'],
  [routeContractDocPath, 'contratto route leggibile', 'Ripristinare docs/launch-v0/route-contract.md.'],
];
...
function routeResultFromChecks(checks) {
  if (checks.some((check) => check.result === 'FAIL')) return 'FAIL';
  if (checks.some((check) => check.result === 'WARN' || check.result === 'MANUAL')) return 'LIMITED';
  return 'PASS';
}

const uxGateDocsReady = uxGateDocs.every(([docPath]) => existsSync(docPath));
const issues = [];
if (!existsSync(contractPath)) {
  issues.push(issue('P0', 'contract-missing', 'Contratto route v0 mancante.', 'Creare docs/launch-v0/v0-routes.json.'));
}

for (const [docPath, label, action] of uxGateDocs) {
  if (!existsSync(docPath)) {
    issues.push(issue('P1', 'ux-copy-gate-doc-missing', `${label} assente.`, action));
  }
}
...
const routeResults = contract.map((route) => {
  ...
  if (route.requiresSourceAndLimits || route.humanCheckRequired) {
    checks.push({ name: 'ux/copy readiness gate', result: route.humanCheckRequired ? 'MANUAL' : uxGateDocsReady ? 'PASS' : 'WARN' });
    if (route.humanCheckRequired) {
      issues.push(issue('P1', 'manual-ux-copy-gate', `${route.path} richiede QA comunicativa: label pubblica, stato v0, CTA, empty state e linguaggio prudente.`, `Eseguire la checklist ${path.relative(root, uxCopyGatePath)} prima della pubblicazione.`, route.path));
    }
  }
  return { ...route, checks, result: routeResultFromChecks(checks) };
});
...
if (uxGateDocsReady) {
  issues.push(issue('INFO', 'ux-copy-gate-docs-present', 'Checklist UX/copy e documenti guida v0 presenti.', `Usare ${path.relative(root, uxCopyGatePath)} per la review manuale.`));
}
...
const payload = {
  schemaVersion: 'v0-readiness/1',
  generatedAt,
  outcome,
  counts: { P0: p0, P1: p1, P2: p2, INFO: countBySeverity(issues, 'INFO') },
  reportPath: path.relative(root, reportPath),
  routeContractPath: path.relative(root, contractPath),
  uxCopyGatePath: path.relative(root, uxCopyGatePath),
  routeResults,
  issues,
};
