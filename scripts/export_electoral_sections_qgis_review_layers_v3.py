...
OUTPUT_GPKG = QA_DIR / "electoral_sections_qgis_review_layers_2025_v3.gpkg"
...
def main() -> int:
    ...
    v3 = gpd.read_file(POLYGONS_V3_GPKG, layer="electoral_sections_candidate_2025_v3_census_aggregated")
    ...
    cells = gpd.read_file(CELL_ASSIGNMENT_GPKG, layer="electoral_section_census_cells_assignment_2025")
    ...
    deterministic = civics_v1[
        (civics_v1["human_review_required"] == False) & (civics_v1["section_number"].astype(str) != "78")
    ].copy()
    ...
    write_gpkg(v3, OUTPUT_GPKG, "candidate_sections_v3_census_aggregated", delete_existing=True)
    write_gpkg(v2, OUTPUT_GPKG, "candidate_sections_v2_voronoi")
    write_gpkg(v1, OUTPUT_GPKG, "candidate_sections_v1_voronoi")
    write_gpkg(cells, OUTPUT_GPKG, "census_cells_assignment")
    ...
    print("qgis review gpkg V3")
    print(f"output={OUTPUT_GPKG}")
    print(f"candidate_sections_v3_census_aggregated={len(v3)}")
    print(f"candidate_sections_v2_voronoi={len(v2)}")
    print(f"candidate_sections_v1_voronoi={len(v1)}")
    print(f"census_cells_assignment={len(cells)}")
