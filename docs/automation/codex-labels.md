...
| `codex:prompted` | A Codex-ready prompt has been generated and posted. | Operational; counts toward capacity 3 while waiting for invocation. | Automation 1. |
| `codex:invoked` | Codex has been invoked. | Operational; counts toward capacity 3. | Automation 2. |
| `codex:working` | Codex work or an implementation PR is in progress. | Operational; counts toward capacity 3 until PR is ready for human review. | Automation 2 or review automation. |
...
- **Operational states that can saturate capacity:** `codex:prompted`, `codex:invoked`, `codex:working`, and open Codex PRs that still need Codex-side changes.
- **Human review wait:** `codex:review-needed`; this is not queue saturation unless there is concrete file/module collision or Codex-side rework.
...
- The operational queue has maximum capacity 3.
...
- The queue governor should not treat `codex:review-needed` as saturation unless a concrete file/module collision exists.
- If there are no operational tasks and no open PRs, the queue governor should apply the anti-empty-queue rule and promote safe technical tasks to `codex:ready` up to partial capacity.
...
- The current label set is the source of truth for queue state; stale comments must not override labels.
...
- operational capacity remains at or below 3 after promotion/invocation;
