...
The refresh cascade has two event paths. Ordinary/manual changes to `main` are covered by the `push` trigger. A trusted PR that is itself merged automatically may not yield a new Actions `push` run because of `GITHUB_TOKEN` recursion rules, so the workflow also listens for `pull_request_target: closed` and refreshes the remaining trusted PRs only when the closed PR was actually merged. The closed PR is explicitly excluded from the arm-auto-merge job. This keeps the cascade event-driven without adding scheduled polling.
