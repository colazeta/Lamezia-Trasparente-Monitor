...
| 02    | Frontend static build / public pages | Confirm key public page shells are included: `/`, `/albo`, `/contratti`, `/proposte-civiche`, `/beni-confiscati`, `/fonti-dati`, `/stato-monitoraggio`.                            | Routes are present in the reviewed frontend and remain readable in local/staging review.                         | Record the missing route as a release blocker or follow-up, depending on severity.             |
...
| 07    | Key data pages                       | During approved human validation, inspect `/albo`, `/contratti`, `/proposte-civiche`, `/beni-confiscati`, `/accesso-civico`, `/performance`, `/legalita`, `/fonti-dati`.           | Pages render with accessible headings, cautious explanatory copy and source/limitation context where applicable. | Stop if pages fail to render; otherwise document the data gap or monitoring need as follow-up. |
