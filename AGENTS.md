...
Automation capacity is issue-invocation capacity, not pull-request-management capacity. Automations should prepare and invoke Codex on non-colliding issues labelled `codex:ready` or `codex:prompted` when there is no recent operative `@codex` invocation and capacity remains available. Open PRs, pending reviews and pending merges are Giovanni's review/merge perimeter; they are informative collision signals only and must not block unrelated Codex invocations.
