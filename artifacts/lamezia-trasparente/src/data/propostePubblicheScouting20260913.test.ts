...
describe("scouted public proposals 13 September 2026 and Scordovillo timeline updates", () => {
  ...
  it("updates the same Scordovillo timeline without inferring deposit, calendarisation, reception or implementation", () => {
    ...
    expect(scordovillo.lastUpdated).toBe("2026-09-21");
    ...
    expect(
      scordovillo.events.some(
        (event) =>
          event.id === "scordovillo-murone-risposta-consiglio-21-settembre" &&
          event.type === "aggiornamento",
      ),
    ).toBe(true);
    expect(scordovillo.events.map((event) => event.type)).not.toContain(
      "deposito",
    );
    expect(scordovillo.events.map((event) => event.type)).not.toContain(
      "calendarizzazione",
