...
const DATA_QUALITY_NOTES = [
  ...
  },
  {
    ...
    lastKnownUpdate: `Calcolato: il feed conserva per ogni CUP data e stato dell'acquisizione OpenCUP; nell'ultima versione pubblicata risultano ${LAMEZIA_PNRR_STATIC_DATA.coverage.projects_with_opencup_fresh} corredi aggiornati, ${LAMEZIA_PNRR_STATIC_DATA.coverage.projects_with_opencup_stale} conservati dall'ultima acquisizione valida e ${LAMEZIA_PNRR_STATIC_DATA.coverage.projects_pending_opencup} in attesa. La materializzazione del ${LAMEZIA_PNRR_STATIC_DATA.metadata.materialized_at} non rappresenta una modifica amministrativa del singolo CUP.`,
    identifierCoverage: `Calcolato: ${LAMEZIA_PNRR_STATIC_DATA.coverage.projects_with_opencup} schede OpenCUP riconciliate tramite CUP identico su ${LAMEZIA_PNRR_STATIC_DATA.coverage.projects_with_cup} progetti comunali dotati di CUP; ogni nuovo CUP viene acquisito automaticamente e, se OpenCUP non è temporaneamente disponibile, resta in attesa di ritentativo. Costo totale disponibile per ${LAMEZIA_PNRR_STATIC_DATA.coverage.projects_with_opencup_total_cost} schede e finanziamento pubblico per ${LAMEZIA_PNRR_STATIC_DATA.coverage.projects_with_opencup_public_funding}. I campi mancanti restano null e non diventano zero.`,
    attachmentAvailability:
