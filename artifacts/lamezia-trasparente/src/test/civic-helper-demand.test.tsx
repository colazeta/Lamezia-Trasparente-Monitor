...
function HelperProbe() {
  ...
  return (
    <div>
      <button type="button" onClick={helper.openAssistant}>
        Apri assistente
      </button>
      <button type="button" onClick={helper.openIntro}>
        Apri introduzione
      </button>
      <button type="button" onClick={helper.dismissWelcome}>
        Chiudi introduzione
      </button>
...
describe("CivicHelperProvider on-demand data", () => {
  ...
  it("does not request guide data until the introduction is opened", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      json: vi.fn().mockResolvedValue(guideResponse("2026-09")),
    });
    ...
    await waitFor(() => {
      expect(screen.getByTestId("guide-version")).toHaveTextContent("2026-09");
    });
