...
// deduplicare le azioni ripetute dalla stessa sorgente (segnali di rilevanza,
// condivisioni), così un singolo utente non può gonfiare i contatori.
function requestSourceKey(req: Request): string {
...
router.post("/themes/:id/share", async (req, res) => {
  ...
  const updated = await db.transaction(async (tx) => {
    ...
    // Inserisce la condivisione solo se questa sorgente non ha già condiviso il
    // tema su questo canale. In caso di duplicato l'inserimento viene ignorato
    // e il contatore resta invariato, così un singolo utente non può gonfiare
    // le condivisioni cliccando ripetutamente.
    const inserted = await tx
      .insert(sharesTable)
      .values({ themeId: id, channel: parsed.data.channel, dedupeKey })
      .onConflictDoNothing({
        target: [
          sharesTable.themeId,
          sharesTable.channel,
          sharesTable.dedupeKey,
        ],
      })
      .returning();
