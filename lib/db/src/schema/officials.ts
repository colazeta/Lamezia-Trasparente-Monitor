...
export const OFFICIAL_STATUSES = ["in_carica", "cessato"] as const;
export type OfficialStatusValue = (typeof OFFICIAL_STATUSES)[number];

export const officialsTable = pgTable(
  "officials",
  {
  ...
  (t) => ({
    statusIdx: index("officials_status_idx").on(t.status),
    roleIdx: index("officials_role_idx").on(t.role),
    statusCheck: check(
      "officials_status_check",
      sql`${t.status} in ('in_carica', 'cessato')`,
    ),
  }),
);

export const officialActivitiesTable = pgTable(
  "official_activities",
  {
    id: serial("id").primaryKey(),
  ...
  (t) => ({
    officialIdIdx: index("official_activities_official_id_idx").on(
      t.officialId,
    ),
  }),
);

export const officialRemunerationsTable = pgTable(
  ...
  (t) => ({
    officialIdIdx: index("official_remunerations_official_id_idx").on(
      t.officialId,
    ),
  }),
);

export const officialDeclarationsTable = pgTable(
  ...
  (t) => ({
    officialIdIdx: index("official_declarations_official_id_idx").on(
      t.officialId,
    ),
  }),
);
...
export const officialVotesTable = pgTable(
  ...
  (t) => ({
    ...
    publicationIdIdx: index("official_votes_publication_id_idx").on(
      t.publicationId,
    ),
    sedutaIdIdx: index("official_votes_seduta_id_idx").on(t.sedutaId),
  }),
);
