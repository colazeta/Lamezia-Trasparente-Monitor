...
export interface LtcedsPilotEventPlan {
  ...
  eventSources: Array<{ eventId: string; sourceId: string; supportRole: "event_support" }>;
...
function temporalBounds(
  ...
  if (
    ...
    ISO_DATE_RE.test(temporal.start) && ISO_DATE_RE.test(temporal.end)
  ) {
    return { start: temporal.start, end: temporal.end };
  }
  if (
    eventForm === "course_of_conduct" &&
    !temporal.start && temporal.end && ISO_DATE_RE.test(temporal.end)
  ) {
...
export function evidenceBasisForPrecision(
  ...
  switch (precision) {
    case "exact_public_site": return "source_stated_named_site";
    case "exact_address": return "source_stated_exact";
    case "street_segment": return "source_stated_street";
    case "neighbourhood": return "source_stated_neighbourhood";
    case "locality": return "source_stated_locality";
    case "municipality":
...
export function assertPilotPublicEvent(payload: LtcedsPilotPublicEvent): void {
  ...
  parseDateTime(payload.updated_at, `${payload.event_id}.updated_at`);
  for (const offence of payload.offences) {
    assertUuidV7(offence.offence_instance_id, "offence_instance_id");
  }
...
export function buildPilotEventPlan(input: {
  ...
  const updatedAt = parseDateTime(payload.updated_at, `${payload.event_id}.updated_at`);
  ...
  const sources: PlannedCrimeSourceRow[] = payload.sources.map((source) => ({
    ...
    title: source.source_type === "law_enforcement_primary"
      ? "Comunicato istituzionale"
      : "Fonte istituzionale",
    url: canonicalSourceUrl(source.url),
    publishedAt: source.published_at
      ? parseDateTime(source.published_at, `${source.source_id}.published_at`)
      : null,
  ...
  return {
    ...
    offences,
    locations,
    eventSources: payload.sources.map((source) => ({
      eventId: payload.event_id,
      sourceId: source.source_id,
      supportRole: "event_support" as const,
    })),
    publicEvent: {
      eventId: payload.event_id,
      schemaVersion: payload.schema_version,
...
export function buildPilotImportPlan(input: {
  ...
  for (const plan of events) {
    if (eventIds.has(plan.event.eventId)) {
      throw new Error(`Duplicate pilot event ID: ${plan.event.eventId}`);
    }
    eventIds.add(plan.event.eventId);
  }
