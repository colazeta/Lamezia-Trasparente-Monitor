...
test("canonical JSON and payload hash are key-order independent", () => {
...
test("deterministic child identity is stable and UUIDv7-shaped", () => {
  ...
  assert.equal(first, deterministicChildUuidV7(eventId, "location:0:occurrence"));
  assert.notEqual(first, deterministicChildUuidV7(eventId, "location:1:occurrence"));
...
test("provider and location mappings fail closed", () => {
  assert.equal(
  ...
  assert.throws(() => providerFromSourceUrl("https://example.invalid/source"), /Unrecognised pilot institutional source host/);
...
test("real pilot plans four events without invented coordinates or dates", async () => {
  const plan = buildPilotImportPlan({ files: await pilotFiles() });
  ...
  const cilea = plan.events.find((item) => item.event.title.includes("via Cilea"));
  assert.ok(cilea);
  assert.equal(cilea.event.temporalStartBound, "2025-10-25");
  assert.equal(cilea.event.temporalEndBound, "2025-10-25");
  assert.equal(cilea.locations[0]?.evidenceBasis, "source_stated_street");
...
test("public projection operation classifier is idempotent", async () => {
  const event = buildPilotEventPlan((await pilotFiles())[0]!);
...
test("execute implementation has transaction and no implicit delete path", async () => {
