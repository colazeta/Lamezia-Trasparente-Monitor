...
/**
 * Removes only the five unchanged fictional rows formerly inserted by the
 * development seed. Every public/editorial field in the original signature is
 * matched deliberately: if a row has subsequently been curated, it survives
 * and can be reviewed by a human instead of being deleted automatically.
 *
 * Keep this statement byte-for-byte aligned with migration 0013. The runtime
 * repair is required for legacy databases that are baselined from
 * `drizzle-kit push`: baselining records data migrations without executing
 * them, so the same idempotent cleanup also runs after normal migrations.
 */
export const LEGACY_CONFISCATED_ASSETS_DEMO_CLEANUP_SQL = `
...
export async function removeLegacyConfiscatedAssetDemoRows(
  client: QueryClient,
): Promise<number> {
  const table = await client.query(
    "SELECT to_regclass('public.confiscated_assets') IS NOT NULL AS present",
  );
  if (!table.rows[0]?.["present"]) return 0;

  const result = await client.query(LEGACY_CONFISCATED_ASSETS_DEMO_CLEANUP_SQL);
